    ...
    buildFollowupHtml(container, limit = null) {
        ...
        data.forEach((item, index) => {
            ...
            if (this.currentUser === 'Owner') {
                ...
                // Manager sees complete button if assigned to them, plus edit is handled via row click
                try {
                    const assignedRole = item.assign ? item.assign.split(':')[0].trim() : '';
...
Kindly arrange the remaining payment at your earliest convenience. Thank you for choosing Artis Studio! 🌟`;
    ...
    renderEmployees() {
        ...
        this.teamMembers.forEach(m => {
            ...
            const remaining = baseSalary - paidSalary;
            ...
            if (remaining <= 0 && baseSalary > 0) {
                statusClass = 'pill-completed';
                statusText = 'Paid';
            } else if (paidSalary > 0 && remaining > 0) {
            ...
            html += `
                <tr style="transition: 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="color: var(--warning); font-weight: 600;">₹${remaining.toLocaleString('en-IN')}</td>
                    <td><span class="f-status-pill ${statusClass}">${statusText}</span></td>
                    <td>
                        <div style="display:flex; gap:8px;">
                            ...
                            ${remaining > 0 ? `
                            <button class="btn-quick team" style="padding: 6px 12px; font-size: 13px; background: rgba(37,211,102,0.1); color:#25d366; border-color:#25d366;" onclick="app.sendSalaryReminder('${m.phone}', '${m.name}', ${remaining})">
                                <i class='bx bxl-whatsapp'></i> Reminder
                            </button>` : ''}
                        </div>
                    </td>
                </tr>
        ...
        tbody.innerHTML = html.length > 0 ? html : `<tr><td colspan="8" style="text-align: center; color: var(--text-muted); padding: 20px;">No employees in directory.</td></tr>`;

        document.getElementById('emp-stat-base').textContent = `₹ ${totalBase.toLocaleString('en-IN')}`;
        document.getElementById('emp-stat-paid').textContent = `₹ ${totalPaid.toLocaleString('en-IN')}`;
        document.getElementById('emp-stat-pending').textContent = `₹ ${(totalBase - totalPaid).toLocaleString('en-IN')}`;
    }
    ...
    saveSalary(e) {
        ...
        this.showAlert(`Salary records for ${emp.name} updated.`, 'Success');
    }
    ...
    renderStudents() {
        ...
        this.students.forEach(s => {
            const fees = parseFloat(s.total) || 0;
            ...
            let statusClass = 'pill-pending';
            if (s.status === 'Active') statusClass = 'pill-progress';
            else if (s.status === 'Completed') statusClass = 'pill-completed';
            else if (s.status === 'Dropped') statusClass = 'pill-pending';

            html += `
                <tr style="transition: 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
                    <td style="font-weight: 600;">${s.name}</td>
                    <td><span class="badge" style="background: rgba(59,130,246,0.1); color: #60a5fa;">${s.course}</span></td>
                    <td style="color: var(--primary); font-weight: 600;">₹${fees.toLocaleString('en-IN')}</td>
                    <td style="color: var(--success); font-weight: 600;">₹${paid.toLocaleString('en-IN')}</td>
                    <td style="color: var(--danger); font-weight: 600;">₹${remaining.toLocaleString('en-IN')}</td>
                    <td>${s.date}</td>
                    <td><span class="f-status-pill ${statusClass}">${s.status}</span></td>
                    <td>
                        <div style="display:flex; gap:8px;">
                            <button class="btn-quick manager" onclick="app.openStudentModal(${s.id})" title="Edit / Pay">
                                <i class='bx bx-edit-alt'></i> Edit
                            </button>
                            ${(remaining > 0 && this.currentUser === 'Owner') ? `
                            <button class="btn-quick team" style="background: rgba(37,211,102,0.1); color:#25d366; border-color:#25d366;" onclick="app.sendFeeReminder('${s.phone}', '${s.name}', '${s.course}', ${fees}, ${paid}, ${remaining})" title="Send Fee Reminder">
                                <i class='bx bxl-whatsapp'></i> Remind
                            </button>` : ''}
                            ${this.currentUser === 'Owner' ? `
                            <button class="btn-quick" style="color: var(--danger); border-color: var(--danger);" onclick="app.deleteStudent(${s.id})" title="Delete Student">
                                <i class='bx bx-trash'></i>
                            </button>` : ''}
                        </div>
                    </td>
        ...
        tbody.innerHTML = html.length > 0 ? html : `<tr><td colspan="8" style="text-align: center; color: var(--text-muted); padding: 20px;">No students found in directory.</td></tr>`;

        document.getElementById('student-stat-total').textContent = `₹ ${totalFees.toLocaleString('en-IN')}`;
        document.getElementById('student-stat-paid').textContent = `₹ ${totalPaid.toLocaleString('en-IN')}`;
        document.getElementById('student-stat-pending').textContent = `₹ ${(totalFees - totalPaid).toLocaleString('en-IN')}`;
    }
    ...
    sendFeeReminder(phone, name, course, total, paid, remaining) {
        if (!phone || phone === '-') {
            this.showAlert('No valid phone number found for this student.', 'Cannot Send WhatsApp');
            return;
        }
        ...
        const msg = `Hello ${name},\n\nThis is a friendly fee reminder from Artis Studio for the "${course}" course.\n\n💰 Course Fees: ₹${total.toLocaleString('en-IN')}\n✅ Amount Paid: ₹${paid.toLocaleString('en-IN')}\n❗ Remaining Due: ₹${remaining.toLocaleString('en-IN')}\n\nKindly clear the pending dues as soon as possible.\nThank you! ✨`;
        window.open(`https://wa.me/${waNum}?text=${encodeURIComponent(msg)}`, '_blank');
    }
    ...
    setupEventListeners() {
        ...
        window.onclick = function (event) {
            ...
            // followup-modal: does NOT close on backdrop click (only Cancel/Save)
